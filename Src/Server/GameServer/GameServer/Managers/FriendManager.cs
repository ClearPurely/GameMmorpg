...
namespace GameServer.Managers
{
    class FriendManager
    {
        ...
        public NFriendInfo GetFriendInfo(TCharacterFriend friend)
        {
            ...
            Log.InfoFormat("{0}:{1} GetFriendInfo:{2}:{3} Status:{4}", this.Owner.Id, this.Owner.Info.Name, friendInfo.friendInfo.Id, friendInfo.friendInfo.Name, friendInfo.Status.ToString());
        ...
        public void PostProcess(NetMessageResponse message)
        {
            if(friendChanged)
            {
                Log.InfoFormat("PostProcess > FriendManager: characterITD:{0}:{1}", this.Owner.Id, this.Owner.Info.Name);
                this.InitFriends();
