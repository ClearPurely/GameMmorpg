...
namespace GameServer.Services
{
    class FriendService : Singleton<FriendService>
    {
        ...
        void OnFriendAddRequest(NetConnection<NetSession> sender, FriendAddRequest request)
        {
            ...
            Log.InfoFormat("OnFrienAddlRequest: :FronId:{0} FromName:{1} ToID:{2} ToName;{3}", request.FromId, request.FromName, request.ToId, request.ToName);
            ...
            Log.InfoFormat("ForwardRequest: :FormId:{0} FormName: {1} ToID:{2} ToName:{3}", request.FromId, request.FromName, request.ToId, request.ToName);
        ...
        {
            Character character = sender.Session.Character;
            Log.InfoFormat("OonFriendaddiResponse: :character:{0} Result:{1} FormId:{2} ToID:{3}", character.Id, response.Result, response.Request.FromId, response.Request.ToId);
